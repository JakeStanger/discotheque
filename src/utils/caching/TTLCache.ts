/**
 * Taken directly from https://github.com/Brokerloop/ttlcache/blob/master/src/index.ts
 * but modified to not bump age on get
 */
...
interface ICacheEntry<K, V> {
  ...
  prev: ICacheEntry<K, V> | null;
  next: ICacheEntry<K, V> | null;
}

export interface ICacheEntryView<K, V> {
  key: K;
  val: V;
}
...
const def = {
  ...
  clock: Date as IClock, // cache-relative clock
};
...
export class TTLCache<K = any, V = any> {
  ...
  readonly evict = new Signal<ICacheEntryView<K, V>>();

  private readonly cache = new Map<K, ICacheEntry<K, V>>();
  private oldest: ICacheEntry<K, V> | null = null;
  private newest: ICacheEntry<K, V> | null = null;
  ...
  *entries() {
    for (const entry of this.getIterator()) {
      const view: ICacheEntryView<K, V> = {
        key: entry.key,
        val: entry.val,
      };
  ...
  private bumpAge(entry: ICacheEntry<K, V>) {
  ...
  private unlink(entry: ICacheEntry<K, V>) {
  ...
  private isExpired<K, V>(entry: ICacheEntry<K, V>) {
